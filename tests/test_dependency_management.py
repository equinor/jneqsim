...
class TestNeqSimDependencyManager:
    ...
    def test_config_loading_invalid_file(self):
        ...
        try:
            with pytest.raises(yaml.YAMLError):
                invalid_config = load_config(invalid_config_path)
                NeqSimDependencyManager(config=invalid_config)
        finally:
    ...
    def test_config_loading_missing_file(self):
        ...
        with pytest.raises(FileNotFoundError):
            nonexistent_config = load_config(nonexistent_path)
            NeqSimDependencyManager(config=nonexistent_config)
